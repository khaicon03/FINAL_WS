...
def apply_tf_2d(pts: np.ndarray, tx: float, ty: float, yaw: float):
...
def line_heading(a: float, b: float):
...
class CorridorCenterCmdVel:
    def __init__(self):
        ...
        # vd rear_to_lidar = 0.20m => x_ref = -0.20
        ...
        rospy.loginfo("corridor_center_cmdvel started.")
    ...
    def cb(self, scan: LaserScan):
        ...
        # filter range by recomputing r from xy (nhanh và đơn giản)
        ...
        # bring into vehicle/control frame (fixed mounting)
        pts = apply_tf_2d(pts, self.laser_x, self.laser_y, self.laser_yaw)

        # split left/right by angle
        ...
        # heading error: average available
        if len(psi_list) == 1:
            epsi = psi_list[0]
        else:
        ...
        # lateral error: center between two walls
        if dL is not None and dR is not None:
            ey = 0.5*(dL - dR)  # >0 nghĩa là lệch về phải (nhiều space bên trái) -> steer left
        elif dL is not None:
        ...
        # control -> steering angle (rad)
        v = max(self.pwm_fwd, self.pwm_min)  # ở đây chỉ dùng làm "scale mềm", vì driver là PWM
