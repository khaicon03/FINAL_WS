...
class CorridorCenterCmdVel(object):
    def __init__(self):
        ...
        # --- CẤU HÌNH TRẠNG THÁI ---
        self.stop_time = 2.0  # Thời gian dừng chờ trước khi đảo chiều
        ...
        # ====== Cấu hình quay tìm biên TURN / FOLLOW BIÊN ======
        self.mode = rospy.get_param("~mode_start", "TURN")  # TURN hoặc FOLLOW

        # lựa chọn quay: "left" hoặc "right" khi đi hết luống quay 180 độ
        self.turn_side = rospy.get_param("~turn_side", "left").lower()
        self.turn_dir = 1.0 if self.turn_side == "left" else -1.0
        
        # thông số di chuyển khi chuyển sang luống khác
        self.wheel_base = float(rospy.get_param("~wheel_base", 0.15))   # bạn yêu cầu, hiện chưa cần dùng
        self.turn_delta = float(rospy.get_param("~turn_delta", 0.4))    # rad
        self.turn_speed = float(rospy.get_param("~turn_speed", 0.15))   # theo driver bạn đang dùng (linear.x)

        # điều kiện bắt biên khi sang luống khác
        self.bound_deg_min = float(rospy.get_param("~bound_deg_min", -40.0))
        self.bound_deg_max = float(rospy.get_param("~bound_deg_max", 10.0))
        self.bound_min_inliers = int(rospy.get_param("~bound_min_inliers", 30))
        # ------- Hết cấu hình ----------
        ...
        # Cấu hình tăng độ chính xác cho điều khiển, quét LIDAR
    ...
    def cb(self, scan):
        ...
        r_all = np.linalg.norm(pts_raw, axis=1)
        ang_all = np.degrees(np.arctan2(pts_raw[:, 1], pts_raw[:, 0]))

        sector = (ang_all >= self.bound_deg_min) & (ang_all <= self.bound_deg_max) & np.isfinite(r_all)
        # dùng cùng range lọc cho ổn định
        sector = sector & (r_all > self.r_min) & (r_all < self.r_max)
        pts_bound = pts_raw[sector]

        bound_line = None
        bound_inliers = 0
        if pts_bound.shape[0] >= 2:
            tmp = ransac_line(pts_bound, self.thresh, self.iters, self.bound_min_inliers)
            if tmp is not None:
                bound_inliers = tmp[3]
                bound_line = tmp

        boundary_ok = (bound_line is not None) and (bound_inliers >= self.bound_min_inliers)
        ...
        if self.mode == "TURN":
            if boundary_ok:
                rospy.loginfo("BAT DUOC BIEN (inliers=%d). CHUYEN SANG FOLLOW.", bound_inliers)
                self.mode = "FOLLOW"
                # (không return, cho chạy tiếp sang FOLLOW ngay)
            else:
                # quay theo lựa chọn left/right
                cmd = Twist()
                cmd.linear.x = self.turn_speed
                cmd.angular.z = self.turn_dir * self.turn_delta  # driver STM32 của bạn hiểu angular.z là góc lái (rad)
                self.pub.publish(cmd)
                return

        # ===== FOLLOW mode: nếu mất biên thì dừng 2s rồi quay lại =====
        if self.mode == "FOLLOW":
            if not boundary_ok:
                rospy.logwarn("MAT BIEN -> DUNG %.1fs ROI QUAY TIM LAI.", self.stop_time)
                self.stop_until = now + rospy.Duration(self.stop_time)
                self.mode = "TURN"
                self.stop()
                return
