...
class CorridorCenterCmdVel(object):
    def __init__(self):
        ...
        # ====== QUAY CUỐI LUỐNG (ACKERMANN SERVO) - THÊM MỚI ======
        # Trigger: chỉ khi MẤT CẢ HAI BÊN (left=None và right=None) liên tiếp N frame
        ...
        # Góc quay mong muốn (độ): 180 cho quay đầu, có thể đổi 90/120...
        self.turn_angle_deg = float(rospy.get_param("~turn_angle_deg", 180.0))

        # Chọn hướng quay: "left" hoặc "right"
        self.turn_direction = str(rospy.get_param("~turn_direction", "left")).strip().lower()
        self.turn_dir_sign = 1.0 if self.turn_direction in ["left", "l", "+", "ccw"] else -1.0

        # Lệnh servo (rad) + tốc độ khi quay (m/s)
        self.turn_steer_angle = float(rospy.get_param("~turn_steer_angle", 0.4))
        self.turn_speed = float(rospy.get_param("~turn_speed", 0.15))

        # Bán kính quay đo thực nghiệm (m). Nếu >0 sẽ ưu tiên dùng để tính thời gian quay.
        self.turn_radius_meas = float(rospy.get_param("~turn_radius_meas", 0.38))
        self.wheelbase = float(rospy.get_param("~wheelbase", 0.15))  # vẫn giữ để fallback nếu cần

        # Trạng thái quay cuối luống
        self.endrow_turning = False
        self.endrow_turn_until = rospy.Time(0)
        self.endrow_lost_count = 0
    ...
    def start_endrow_turn(self, now):
        """Bắt đầu quay cuối luống theo Ackermann/bicycle (servo angle).
        Driver hiểu cmd.angular.z là góc servo (rad).
        Thời gian quay ưu tiên dùng bán kính đo thực nghiệm: T = angle_rad * R / v.
        """
        angle_rad = math.radians(max(1.0, float(self.turn_angle_deg)))  # tránh 0
        v = max(float(self.turn_speed), 0.01)

        if float(self.turn_radius_meas) > 1e-3:
            duration = angle_rad * float(self.turn_radius_meas) / v
        else:
            # Fallback: dùng mô hình bicycle với wheelbase và góc lái (coi như góc bánh)
            L = max(float(self.wheelbase), 1e-3)
            delta = max(min(float(self.turn_steer_angle), 1.2), -1.2)  # tránh tan bùng
            omega = v * math.tan(abs(delta)) / L
            omega = max(omega, 1e-3)
            duration = angle_rad / omega

        self.endrow_turning = True
        self.endrow_turn_until = now + rospy.Duration(duration)
        self.endrow_lost_count = 0
    ...
    def cb(self, scan):
        ...
        # --- QUAY CUỐI LUỐNG ĐANG DIỄN RA ---
        if self.endrow_turning:
            if now < self.endrow_turn_until:
                cmd = Twist()
                cmd.linear.x = float(self.turn_speed)
                cmd.angular.z = float(self.steer_sign) * float(self.turn_dir_sign) * float(self.turn_steer_angle)
                self.pub.publish(cmd)
                return
            else:
                self.endrow_turning = False
        ...
        if left is None and right is None:
            # Mất cả 2 bên: coi như đi hết luống -> kích hoạt quay theo Ackermann (servo) nếu bật
            if self.enable_endrow_turn and (not self.reversed) and (not self.pending_reverse) and (not self.pending_forward) and (now >= self.stop_until):
                self.endrow_lost_count += 1
                if self.endrow_lost_count >= self.lost_frames_to_turn:
                    self.start_endrow_turn(now)
                    cmd = Twist()
                    cmd.linear.x = float(self.turn_speed)
                    cmd.angular.z = float(self.steer_sign) * float(self.turn_dir_sign) * float(self.turn_steer_angle)
                    self.pub.publish(cmd)
                    ...
                    # Chờ đủ số frame xác nhận mất bám để tránh nhiễu
                    self.stop()
                    return
            else:
                self.stop()
        ...
        # Có ít nhất 1 bên còn nhìn thấy -> reset đếm mất bám cuối luống
        self.endrow_lost_count = 0
