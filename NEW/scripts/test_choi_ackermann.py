...
class CorridorCenterCmdVel(object):
    def __init__(self):
        ...
        # ====== CẤU HÌNH QUAY 90 ĐỘ (ACKERMANN) ======
        # Khi đi đến cuối luống (không còn thấy tường/điểm bám), xe sẽ tự quay trái 90° để vào luống mới.
        self.enable_turn = bool(rospy.get_param("~enable_turn", True))
        self.lost_frames_to_turn = int(rospy.get_param("~lost_frames_to_turn", 8))  # số frame mất bám liên tiếp để kích hoạt quay
        self.lost_count = 0

        # Tham số hình học Ackermann
        self.wheelbase = float(rospy.get_param("~wheelbase", 0.32))      # (m) khoảng cách trục trước-sau
        self.turn_radius = float(rospy.get_param("~turn_radius", 0.60))  # (m) bán kính quay mong muốn
        self.turn_speed = float(rospy.get_param("~turn_speed", 0.15))    # (m/s) tốc độ khi quay

        # Nếu driver của bạn hiểu cmd.angular.z là yaw-rate (rad/s) thì đặt True.
        # Nếu driver hiểu cmd.angular.z là góc lái (rad) thì để False (mặc định).
        self.publish_yaw_rate = bool(rospy.get_param("~publish_yaw_rate", False))

        # Trạng thái quay
        self.turning = False
        self.turn_until = rospy.Time(0)
        self.turn_delta = 0.0
        self.turn_yaw_rate = 0.0
...
def start_turn_left_90(self, now):
    """Bắt đầu quay trái 90° theo mô hình Ackermann (hình học)."""
    # Tính góc lái theo hình học Ackermann: delta = atan(L / R)
    L = max(self.wheelbase, 1e-3)
    R = max(self.turn_radius, 1e-3)

    delta = math.atan2(L, R)  # góc lái dương = quay trái
    # Giới hạn theo delta_max của hệ thống
    delta = max(-self.delta_max, min(self.delta_max, delta))

    # Yaw rate theo mô hình bicycle/Ackermann: omega = v * tan(delta) / L
    v = abs(self.turn_speed)
    omega = v * math.tan(delta) / L

    # Thời gian để đổi hướng 90°: T = (pi/2) / omega
    T = (math.pi / 2.0) / max(abs(omega), 1e-3)

    self.turning = True
    self.turn_until = now + rospy.Duration(T)
    self.turn_delta = delta
    self.turn_yaw_rate = omega

    rospy.loginfo("END-OF-ROW -> TURN LEFT 90deg: delta=%.2fdeg, omega=%.2frad/s, T=%.2fs",
                  math.degrees(delta), omega, T)

def publish_turn_cmd(self):
    cmd = Twist()
    cmd.linear.x = abs(self.turn_speed)  # luôn tiến khi quay vào luống mới
    if self.publish_yaw_rate:
        cmd.angular.z = self.turn_yaw_rate  # rad/s
    else:
        cmd.angular.z = self.turn_delta     # rad (góc lái)
    self.pub.publish(cmd)
    ...
    def cb(self, scan):
        ...
        # --- LOGIC CHO XE ĐANG LÙI (REVERSE) - MỚI ---
            ...
            if int(rear_danger.sum()) >= self.stop_min_points:
                ...
                return
        # ======================================================================
        # 2.5. TRẠNG THÁI QUAY 90 ĐỘ (END-OF-ROW TURN)
        # ======================================================================
        if self.turning:
            if now < self.turn_until:
                ...
                self.turning = False
                self.lost_count = 0
                rospy.loginfo("KET THUC QUAY 90 DO, BAT DAU BAM LUONG MOI.")
        ...
        if pts.shape[0] < 80:
            ...
            if (self.enable_turn and (not self.reversed) and (not self.pending_reverse) and (not self.pending_forward)
                    and (not self.turning) and (self.lost_count >= self.lost_frames_to_turn)):
        ...
        # Có ít nhất 1 bên bám được -> reset bộ đếm mất bám
        self.lost_count = 0
