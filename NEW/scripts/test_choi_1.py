...
class CorridorCenterCmdVel(object):
    def __init__(self):
        ...
        # ====== QUAY CUỐI LUỐNG: QUAY TRÁI 90° (ACKERMANN) ======
        # Khi mất cả hai tường/điểm bám liên tiếp một số frame, xe sẽ quay trái 90° để vào luống mới.
        self.enable_turn = bool(rospy.get_param("~enable_turn", True))
        self.lost_frames_to_turn = int(rospy.get_param("~lost_frames_to_turn", 8))
        self.lost_count = 0

        # Tham số hình học/điều khiển khi quay
        self.wheelbase = float(rospy.get_param("~wheelbase", 0.15))           # (m) khoảng cách trục trước-sau
        self.turn_steer_angle = float(rospy.get_param("~turn_steer_angle", 0.40))  # (rad) góc lái servo cố định khi quay trái 90°
        self.turn_speed = float(rospy.get_param("~turn_speed", 0.15))         # (m/s) tốc độ khi quay

        self.turn_radius_meas = float(rospy.get_param("~turn_radius_meas", 0.38)) # (m) bán kính quay đo thực tế (nếu >0 sẽ dùng để tính thời gian quay 90°)
        self.turn_delta_max = float(rospy.get_param("~turn_delta_max", 0.60))   # (rad) giới hạn góc lái servo khi quay
        # Trạng thái quay
        self.turning = False
    ...
    def start_turn_left_90(self, now):
        """Bắt đầu quay trái 90° với góc lái servo cố định self.turn_steer_angle (rad).
        Driver hiểu cmd.angular.z là góc lái servo, nên ta publish góc lái; chỉ dùng Ackermann để tính thời gian quay.
        """
        # đảm bảo thông số hợp lệ
        L = max(float(self.wheelbase), 1e-3)

        # Góc lái servo cố định khi quay trái (rad)
        delta = abs(float(self.turn_steer_angle))  # quay trái: dùng góc dương

        # Giới hạn riêng cho chế độ quay (KHÔNG dùng delta_max của điều khiển bám luống)
        delta_lim = abs(float(getattr(self, "turn_delta_max", 0.60)))
        if delta_lim > 1e-6:
            delta = max(0.0, min(delta_lim, delta))

        v = max(float(self.turn_speed), 0.01)  # tránh chia 0

        # Nếu có bán kính quay đo thực tế -> dùng để tính thời gian quay 90° (tin cậy hơn open-loop Ackermann)
        R_meas = float(getattr(self, "turn_radius_meas", 0.0))
        if R_meas > 1e-3:
            duration = (math.pi / 2.0) * R_meas / v
        else:
            # fallback: bicycle/ackermann kinematics
            omega = v * math.tan(delta) / L
            omega = max(abs(omega), 1e-3)  # tránh omega quá nhỏ => thời gian vô hạn
            duration = (math.pi / 2.0) / omega  # 90° / yaw_rate
        self.turning = True
        self.turn_until = now + rospy.Duration(duration)
        self.turn_delta = delta

        rospy.logwarn("END-OF-ROW: turn left 90deg | delta=%.3frad (servo) | v=%.2fm/s | T=%.2fs",
                      delta, v, duration)

    def publish_turn_cmd(self):
        cmd = Twist()
        cmd.linear.x = float(self.turn_speed)
        # Driver hiểu angular.z là góc lái servo (rad)
        cmd.angular.z = float(self.turn_delta) * float(self.steer_sign)
        self.pub.publish(cmd)
    ...
    def cb(self, scan):
        ...
        # --- ƯU TIÊN CHẾ ĐỘ QUAY CUỐI LUỐNG ---
        if self.turning:
            if now < self.turn_until:
                # an toàn: vẫn kiểm tra vật cản phía trước khi đang quay
                ...
                if int(front_danger.sum()) >= self.stop_min_points:
                    rospy.logwarn("VAT CAN KHI DANG QUAY! DUNG VA HUY LENH QUAY.")
                    self.turning = False
                    self.stop()
                    return

                self.publish_turn_cmd()
                return
            else:
                # quay xong
                self.turning = False
                self.lost_count = 0
                rospy.loginfo("DA QUAY XONG 90 DO. TIEP TUC BAM LUONG MOI.")
        ...
        if pts.shape[0] < 80:
            # Không đủ điểm trong ROI -> coi như mất cả 2 bên (tới cuối luống)
            if self.enable_turn and (not self.reversed) and (not self.pending_reverse) and (not self.pending_forward) and (not self.turning):
                self.lost_count += 1
                if self.lost_count >= self.lost_frames_to_turn:
                    self.start_turn_left_90(now)
                    self.publish_turn_cmd()
                    return
                else:
                    self.stop()
        ...
        if left is None and right is None:
            # Mất cả 2 tường/điểm bám -> coi như tới cuối luống
            if self.enable_turn and (not self.reversed) and (not self.pending_reverse) and (not self.pending_forward):
                self.lost_count += 1
                if self.lost_count >= self.lost_frames_to_turn:
                    # kích hoạt quay trái 90°
                    self.start_turn_left_90(now)
                    self.publish_turn_cmd()
                    return
                else:
                    # chờ thêm vài frame để chắc chắn (tránh nhiễu), tạm dừng
                    self.stop()
                    return
            else:
                self.stop()
            ...
            # còn thấy tường -> reset đếm mất bám
            self.lost_count = 0
